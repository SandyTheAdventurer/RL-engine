...
def main():
    ...
    while True:
        ...
        for agent in league.population:
            ...
            logger.info("Agent %d hybrid score: %.4f (style: %.4f [disc: %.4f], outcome: %.4f [dmg_ratio: %.4f vs human: %.4f]) | damage: %.2f", agent.id, agent.score, style_score, gen_score, outcome_score, gen_damage_ratio, human_damage_ratio, gen_damage)
            log_metrics({
