...
def main():
    ...
    while True:
        ...
        def _train_agent(agent):
            logger.info(
                "Agent %d [a=%.2f, ent=%.3f] (GenLR: %.2e, DiscLR: %.2e) started training",
                agent.id, getattr(agent, "reward_alpha", 0.5), getattr(agent, "ent_coef", 0.03), agent.gen_lr, agent.disc_lr,
            )
            ...
            outcome_score = 1.0 - abs(gen_damage_ratio - human_damage_ratio)
            ...
            return agent, style_score, gen_score, outcome_score, gen_damage_ratio, human_damage_ratio, gen_damage
        ...
        for agent, style_score, gen_score, outcome_score, gen_damage_ratio, human_damage_ratio, gen_damage in train_results:
            logger.info("Agent %d [a=%.2f, ent=%.3f] hybrid score: %.4f (style: %.4f [disc: %.4f], outcome: %.4f [dmg_ratio: %.4f vs human: %.4f]) | damage: %.2f", agent.id, getattr(agent, "reward_alpha", 0.5), getattr(agent, "ent_coef", 0.03), agent.score, style_score, gen_score, outcome_score, gen_damage_ratio, human_damage_ratio, gen_damage)
            log_metrics({
